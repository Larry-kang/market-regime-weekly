---
title: TAIEX 階段分析
date: 2026-07-06
---

# TAIEX

<div class="thread-feed">
  <div class="thread-card thread-connector">
    <div class="thread-meta"><span class="thread-avatar"></span><span class="thread-badge">目前結論</span><span>TAIEX / 過熱</span></div>
    <p>**階段：過熱**。先等回檔，不追高。</p>
  </div>

  <div class="thread-card thread-connector">
    <div class="thread-meta"><span class="thread-avatar"></span><span class="thread-badge">一句話</span><span>台股加權常常跑得比想像快，趨勢強時很強，但乖離擴大後也容易變得難追。</span></div>
    <p>乖離過大 / 追高風險 是現在最需要看的重點。</p>
  </div>
</div>

## 現況摘要
| 項目 | 數值 | 解讀 |
|---|---|---|
| 最新收盤 | 47,153.46（2026-07-06） | 最新可用收盤價 |
| 20D / 50D / 200D MA | 45,641.79 / 43,517.96 / 33,361.53 | 日線均線位置 |
| 20W / 50W / 200W MA | 39,785.63 / 32,382.53 / 22,245.60 | 週線均線位置 |
| 距 200W MA | 24,907.86 點（+111.97%） | 距離長週線的乖離 |
| 日線 RSI(14) | 61.0，中性偏強 | 短線動能 |
| 日線 MACD(12,26,9) | MACD 線低於訊號線，柱狀體負63.76，較前值收斂 | 日線趨勢 |
| 週線 RSI(14) | 77.3，超買 | 中週期動能 |
| 週線 MACD(12,26,9) | MACD 線高於訊號線，柱狀體正426.02，較前值收斂 | 週線趨勢 |

## 日線觀察
- RSI 61.0，中性偏強；MACD 線低於訊號線，柱狀體負63.76，較前值收斂
- 日線目前偏向 中性偏強 區，代表短線動能已經有方向，但未必足以單獨當作進場依據。
- 若日線 MACD 柱狀體持續收斂，通常表示短線壓力正在減輕；若擴大，代表還要再等。

## 週線觀察
- RSI 77.3，超買；MACD 線高於訊號線，柱狀體正426.02，較前值收斂
- 週線更能反映中期趨勢，因此這一段通常比日線更值得當成主要判斷。
- 若週線 RSI 已在超買，或價格遠離 200W MA，代表趨勢雖強，但追價容錯率會明顯下降。

## 階段判讀
| 階段 | 典型訊號特徵 | 目前是否接近/位於 |
|---|---|---|
| 熊底 | 跌破 200W、20W/50W/200W 空頭排列、RSI 低檔、MACD 負值擴大 | 否 |
| 過渡 | 仍未完全站穩 200W，或剛開始靠近，動能改善但還不完整 | 否 |
| 復甦 | 收復 200W，短中期均線開始翻揚，動能回到正向 | 否 |
| 牛初 | 20W > 50W > 200W，價格站穩長均線，RSI 多在中性到偏強 | 部分符合 |
| 過熱 | 漲幅與乖離擴大，週 RSI 進入超買，追價風險高 | 是，最貼近目前狀態 |

## 實務意思
- **目前階段**：過熱
- **對應動作**：先等回檔，不追高
- **風險重點**：乖離過大 / 追高風險
- **我的解讀**：台股加權常常跑得比想像快，趨勢強時很強，但乖離擴大後也容易變得難追。

## 週報紀錄
- [2026-07-06 台灣市場週報](../weekly/2026-07-06.md)
