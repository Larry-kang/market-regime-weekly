...
1. **週期階段**：20／50／200 日與週均線、RSI(14)、MACD、距離 200W 均線，用於熊底／過渡／復蘇／牛初／過熱分類。
2. **日報式短線 overlay**：最近 7 日報酬、3／5／7 日均線、短樣本 RSI(6)、最近 7 日高低區間、成交量相對 7 日均值、短線支撐／壓力與資料限制。

短線 overlay 是輔助層，不會覆蓋週期階段；例如同一標的可以同時呈現「週期階段：復蘇」與「日線：偏多震盪／接近壓力」。

## 報告產出與部署

- **價格資料更新**：獨立的 `update-market-data` workflow 於週一至週五台灣時間約 13:30 更新 `data/market_history/`，有變更才 commit／push。
- **日報**：價格更新成功後自動產出 `docs/daily/YYYY-MM-DD.md`，使用價格更新 workflow 已寫入 repo 的資料；也保留手動執行入口。平日不再另跑一組 14:05 排程，避免與資料更新搶同一 concurrency。
